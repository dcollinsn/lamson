                ...
                if ch in vowels:
                    ...
                    if pos == first:  # all init vowels now map to 'A'
                        ...
                        if (pos > first) and (st[pos-2] in vowels) and (st[pos-1:pos+1] == 'ACH') and \
                            ...
                            elif st[first:first+4] in ['VAN ', 'VON '] \
                                    or st[first:first+3] == 'SCH' \
                                    or st[pos-2:pos+4] in ["ORCHES", "ARCHIT", "ORCHID"] \
                                    or st[pos+2] in ['T', 'S'] \
                                    or ((st[pos-1] in ["A", "O", "U", "E"] or pos == first) and st[pos+2] in ["L", "R", "N", "M", "B", "H", "F", "V", "W"]):
                                nxt = ('K', 1)
                            else:
                                if pos == first:
                            ...
                            if pos > first and st[pos-1] not in vowels:
                                ...
                                if st[pos+1] == 'L':
                                    ...
                                    if (pos == (last - 2) and st[pos-1:pos+3] in ["ILLO", "ILLA", "ALLE"]) \
                                            or (st[last-1:last+1] in ["AS", "OS"] or st[last] in ["A", "O"] and st[pos-1:pos+3] == 'ALLE'):
                                        nxt = ('L', ' ', 2)
                                    else:
                                        nxt = ('L', 2)
                                else:
